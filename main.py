...
async def takror_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1) Try to match prepared options robustly (handles emoji prefixes / slight wording differences)
    for k, v in rep_map.get(lang, {}).items():
        if k == text or k in text or text in k:
            td = v
            break

    # 2) Manual entry option (keyboard may include emoji like "✍️ Qo'lda")
    if td is None and (("Qo'lda" in text) or ("Вручную" in text) or ("Qo'lda" in text.replace("✍️", ""))):
        await send(
            update,
    ...
    # 4) If editing, reschedule and show item
    if "edit_target" in users[uid]:
        await reschedule_task(uid, target, context)
        return await tahrir_item_display(update, context)

    # 5) Ask about pre-reminder (unchanged)
    await send(
        update,
        get_s(uid, "ask_pre"),
        ReplyKeyboardMarkup(
...
async def oldindan_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1️⃣ Qo‘lda kiritish
    if (lang == "UZ" and "qo'lda" in text) or (lang == "RU" and "вручную" in text):
        await send(
            update,
            "✍️ Masalan: 10 daqiqa yoki 1 soat:"
            if lang == "UZ"
            else "✍️ Например: 10 минут или 1 час:",
            ReplyKeyboardMarkup([[get_s(uid, "btn_back")]], resize_keyboard=True)
        )
        return OLDINDAN

    # 2️⃣ Raqamni ajratish
    ...
    # 3️⃣ Birliklarni aniqlash
    if lang == "UZ":
        if "daqiqa" in text or "min" in text:
            pre = n
        elif "soat" in text:
            pre = n * 60
        elif "kun" in text:
